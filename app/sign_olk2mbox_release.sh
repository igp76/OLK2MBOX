...
# Stability: Stable
# Change-Summary: Sign and verify the republished clean-root app release inputs.
...
FILES=(
    "$DIST_ROOT/OLK2MBOX-${APP_VERSION}-arm64.zip"
    "$DIST_ROOT/OLK2MBOX-${APP_VERSION}-arm64.dmg"
    "$DIST_ROOT/OLK2MBOX-${APP_VERSION}-release-manifest.json"
    "$DIST_ROOT/OLK2MBOX-${APP_VERSION}-SHA256SUMS.txt"
)

usage() {
    echo "Usage: $0 [--verify-only] [--version]"
    echo
    echo "Create or verify detached OpenPGP signatures for release ${APP_VERSION}."
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --verify-only)
            ...
            ;;
        --version)
            echo "OLK2MBOX release signer ${SCRIPT_VERSION} (app ${APP_VERSION})"
            exit 0
            ;;
        --help|-h)
            usage
...
(cd "$DIST_ROOT" && shasum -a 256 -c "OLK2MBOX-${APP_VERSION}-SHA256SUMS.txt")
...
echo "All release checksums and OpenPGP signatures are valid."
